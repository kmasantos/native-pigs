                    ...
                    <table class="centered striped">
                        ...
                        <tbody>
                            @forelse($breeds as $breed)
                                <tr>
                                    <td>{{ $breed->breed }}</td>
                                    <td><a href="#edit_breed{{ $breed->id }}" class="tooltipped modal-trigger"
                                            data-position="top" data-tooltip="Edit"><i class="fas fa-pencil-alt"></i></a>

                                        @if (empty($breed->deleted_at))
                                            <a href="#delete_breed{{ $breed->id }}" class="tooltipped modal-trigger"
                                                data-position="top" data-tooltip="Delete breed?"><i
                                                    class="fas fa-trash-alt"></i></a>
                                        @else
                                            <a href="#delete_breed{{ $breed->id }}" class="tooltipped modal-trigger"
                                                data-position="top" data-tooltip="Restore breed?"><i
                                                    class="fas fa-trash-restore-alt"></i></a>
                                        @endif
                                    </td>
                                </tr>
                                {{-- MODAL STRUCTURE --}}
                                <div id="delete_breed{{ $breed->id }}" class="modal">
                                    {!! Form::open(['route' => 'admin.delete_breed', 'method' => 'post']) !!}
                                    <div class="modal-content">
                                        <h5 class="center">{{ $breed->deleted_at ? 'Restore' : 'Delete' }} Breed
                                            {{ $breed->breed }}</h5>
                                        <input type="hidden" name="breed_id" value="{{ $breed->id }}">
                                    </div>
                                    <div class="row center">
                                        <button class="btn waves-effect waves-light green darken-3" type="submit">
                                            {{ $breed->deleted_at ? 'Restore' : 'Delete' }} <i
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="row center">
                                            <div class="input-field col s8 m8 l8 offset-s2 offset-m2 offset-l2">
                                                <input id="name" type="text" name="breed" class="validate"
                                                    value="{{ $breed->breed }}" required>
